...
public class DatePickerFactory {
	...
	public static void showDatePicker(final JTextField jtf , int screen_x, int screen_y){
		final JDatePanel datePanel = JDateComponentFactory.createJDatePanel(new UtilDateModel(new Date()));
					datePanel.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent e)
            {
                try{
                jtf.setText(new SimpleDateFormat("yyyy-MM-dd").format(datePanel.getModel().getValue()));
		...
		JDialog dialog = new JDialog(ToolUI.getIntance());
		dialog.setTitle("时间");
